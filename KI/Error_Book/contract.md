# Error_Book — 项目级错题本接口契约

> **本文件是接口定义，不存放任何项目的实际错题内容。**
> 各项目根据本契约在自己的目录中创建 Error_Book 实例。
...
与 Internal_KI 的区别：
- Internal_KI = 项目的正向知识（应该怎么做）
- Error_Book = Agent 的负向记录（不应该怎么做）
...
```json
{
    ...
    "errorCodeReference": {
        "BUILD-001": "CompileFailure",
        "BUILD-002": "TestRegression",
        "BUILD-003": "TypeCheckFailure",
        "BUILD-004": "RuntimeException",
        "REQ-001": "ScopeCreep",
        "REQ-002": "ACNotMet",
        "BHV-001": "FunctionalDefect",
        "BHV-002": "EdgeCaseFailure",
        "ISO-001": "UnauthorizedChange",
        "EVD-001": "ManifestMissing"
    }
}
```
...
项目 CLAUDE.md 中必须声明 Error_Book 路径：
```markdown
## Error Book
- **路径**: `.claude/error_book/`
- **索引**: `.claude/error_book/index.json`
```

全局 Agent Skill 优先读取 Error_Book index.json 做关键词匹配，命中后加载详情。
